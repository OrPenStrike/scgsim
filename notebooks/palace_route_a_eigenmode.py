...
# %%
OUTPUT_CONTROLS = {"run_dir": Path(".artifacts") / "public_route_a_eigenmode"}
...
component = gf.Component("public_route_a_rounded_arm_gap")
...
adjacent = port_entry["metadata"]["adjacent_solution_volume_ids"]
...
sa_attributes = {
    attribute
    for row in config["Boundaries"]["Postprocessing"]["Dielectric"]
    if row["Type"] == "SA"
    for attribute in row["Attributes"]
}
assert not set(lumped_port[0]["Attributes"]).intersection(sa_attributes)
