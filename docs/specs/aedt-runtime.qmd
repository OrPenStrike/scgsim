...
source vacuum record. The Region exterior has an explicit Perfect E boundary.
Substrate domains stay separate, and electric/magnetic domain integrals count
the physical Region once alongside each substrate domain; logical vacuum
components cannot multiply the energy denominator. Historical per-component
result records remain readable under their original source contract.
...
top/bottom `contribution_catalog`; the selected contribution and source polygon
are then checked against that catalogue before constructing `SurfaceEprSpec`. Supply
...
nanometres. A `PartNN` suffix appears only for multiple disconnected pieces.
