...
def prepare_handoff(
    *,
    ...
    script_path.write_text(
        _render_script(profile, executable, resource_map, setup, petsc) + "\n",
        encoding="utf-8",
    )
    script_path.chmod(script_path.stat().st_mode | 0o755)
    hashes = _hashes(
        (mesh_result.mesh_path, config_path, mesh_result.mesh_manifest_path)
    )
    identity = _identity()
    _write_json(
        metadata_path,
    ...
    # It is intentionally outside the archive: it contains the archive hash.
...
def _render_script(
    ...
    for setup_command in setup:
        lines.extend(["set +u", setup_command, "set -u"])
    lines.append(f"{command} 2>&1 | tee logs/palace-${{SLURM_JOB_ID:-manual}}.log")
    return "\n".join(lines)
...
def _hashes(paths: Sequence[Path]) -> list[dict[str, str | int]]:
    return [
